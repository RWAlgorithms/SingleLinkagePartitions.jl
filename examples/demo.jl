...
distance_set, partition_set = SL.runsinglelinkage(
    ...
    early_stop_distance = 5.023150248368374, # set to Inf to get the entire partition tree.
)

# this should be X.
Z0 = SL.instantiatepartition(partition_set[begin], X)

# this should be the singleton partition.
Z_end = SL.instantiatepartition(partition_set[end], X)
