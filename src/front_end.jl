...
function computemean(x::T)::T where T <: Real
    return x
end
...
function mergepoints(
    ...
    Y, _, tol_satisfied = mergepoints(X, Zs, metricfunc; tol = tol)
    
    return Y, tol_satisfied
end

# TODO I am here. trace this.
"""
function mergepoints(
    X::Vector{Vector{T}},
...
function mergepoints(
    ...
    metricfunc::Function;
    tol = 1e-6,
    )::Tuple{Vector{Vector{T}}, Vector{Vector{ZT}}, Bool} where {T,ZT}
...
function mergepoints(
    ...
    metricfunc::Function;
    tol = 1e-6,
    )::Tuple{Vector{Vector{T}}, Vector{ZT}, Bool} where {T,ZT}

    Zs = Vector{Vector{ZT}}(undef, 1)
    Zs[begin] =  Z
    Y, Ys, tol_satisfied, _ = mergepointsfull(X, Zs, metricfunc; tol = tol)
...
function mergepointsfull(
    ...
    metricfunc::Function;
    tol = 1e-6,
    )::Tuple{Vector{Vector{T}}, Vector{Vector{ZT}}, Bool, Vector{Vector{Vector{Int}}}, Vector{T}, Int} where {T,ZT}
